...
object BuildSettings {
  ...
  val basicSettings = Defaults.defaultSettings ++ scaldingSettings ++ printcpSettings ++ Seq (
    name          := Name,
    version       := Version,
    scalaVersion  := ScalaVersion,
    organization  := "de.avantgardelabs",
    description   := "Big Data User Group",
...
object Dependency {
  ...
  val lucene_core    = "org.apache.lucene" % "lucene-core"              % Version.Lucene
  val lucene_analyzr = "org.apache.lucene" % "lucene-analyzers-common"  % Version.Lucene
...
object Dependencies {
  ...
  val bdugscalding = Seq(
    scalaCompiler, scalding_args, scalding_core, scalding_date,
    algebird_core, algebird_util, lucene_core, lucene_analyzr,
    hadoop_core, slf4j, scalaTest)
}

object BigDataUserGroupScaldingBuild extends Build {
  import Resolvers._
  import Dependencies._
  import BuildSettings._

  lazy val bdugscalding = Project(
    id = "BigData-UserGroup-Scalding",
    base = file("."),
    settings = buildSettings ++ Seq(
      // runScriptSetting,
      resolvers := allResolvers,
      libraryDependencies ++= Dependencies.bdugscalding,
